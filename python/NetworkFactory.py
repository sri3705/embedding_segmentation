...
class Network:
    ...
    def createEmbeddingNetwork(self, database_list_path='.', batch_size=20, phase=0):
    	...
    	self.net.inner_product_target_1 = self.getInnerProduct('target', 'inner_product_target_1', 2, num_output=128)
    	self.net.inner_product_negative_1 = self.getInnerProduct('negative', 'inner_product_negative_1', 2, num_output=128)
    	for i in range(0, self.number_of_neighbors):
    		layer = self.getInnerProduct('neighbor{0}'.format(i), 'inner_product_neighbor{0}_1'.format(i), 2, num_output=128)
